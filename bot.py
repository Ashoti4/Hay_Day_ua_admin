...
async def add_rep(message: Message):
    ...
    db[uid].setdefault("msg_history", []) # Захист від старих багів
...
async def set_filter(message: Message, command: CommandObject):
    ...
    if not command.args or not message.reply_to_message:
        return await message.answer(f"Відповідай на повідомлення командою <code>/filter слово</code>{get_footer()}", parse_mode="HTML")
    ...
    await message.answer(f"✅ Фільтр '<b>{trigger}</b>' збережено!{get_footer()}", parse_mode="HTML")
